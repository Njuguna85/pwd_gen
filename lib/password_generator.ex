defmodule PasswordGenerator do
  @moduledoc """
  Generate random password depending on user input params
  Example options:
    options = %{
      "length"=> "5",
      "numbers"=>"false",
  ...
  defp validate_length(false, _options), do: {:error, "Please provide a length for the password"}

  defp validate_length(true, options) do
    numbers = Enum.map(0..9, &Integer.to_string(&1))
    length = options["length"]
    length_is_in_numbers = String.contains?(length, numbers)

    validate_length_is_integer(length_is_in_numbers, options)
  end

  defp validate_length_is_integer(false, _options),
    do: {:error, "Only integers are allowed for length"}
  ...
  defp validate_options_values_are_boolean(false, _length, _options) do
    {:error, "Only booleans are allowed for option values"}
  end
  ...
  defp validate_options(true, _length, _options) do
    {:error, "Only options allowed are numbers, uppercase and symbols"}
  end
